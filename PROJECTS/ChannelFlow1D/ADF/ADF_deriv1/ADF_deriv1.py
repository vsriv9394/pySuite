...
lib = C.CDLL("/home/vsriv/codes/pySuite/PROJECTS/ChannelFlow1D/ADF/ADF_deriv1/libADF_deriv1.so")

class ADF_deriv1(C.Structure):

	_pack_ = 1
	_fields_ = [
		("___phi", C.c_double*16),
		("___psi", C.c_double*16),
		("uL", C.POINTER(VarAD)),
		("uC", C.POINTER(VarAD)),
		("uR", C.POINTER(VarAD)),
		...
		("dudy", C.POINTER(VarAD)),
	]

	C_init = lib.init__ADF_deriv1
	C_init.restype = None
	C_init.argtypes = [
		C.c_void_p,
	]

	C_setVars = lib.setVars__ADF_deriv1
	...
	C_calculate = lib.calculate__ADF_deriv1
	C_calculate.restype = None
	C_calculate.argtypes = [
		C.c_void_p,
	]

	C_setup = lib.setup__ADF_deriv1
